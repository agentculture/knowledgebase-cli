...
knowledgebase-cli vendors its `.claude/skills/` from **guildmaster** — the
...
#   - consumer-identifying prose: `guildmaster` → `knowledgebase-cli` (NOT
...
If a re-sync would lose a knowledgebase-cli adaptation, lift the change
...
#     → `which knowledgebase-cli vendors from guildmaster`
