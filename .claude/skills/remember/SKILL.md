...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope knowledgebase-cli
...
scope:** the default personal scope is **private** (`--scope knowledgebase-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `knowledgebase-cli`). Private records are served only to a recall in the same
